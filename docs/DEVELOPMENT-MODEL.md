...
| LLM context (horizontal) | `stubborn` index, context, metrics | Python |
...
`stubborn` is **outside** the migration SSOT → rewrite → verify path but follows the same philosophy: SQLite snapshots, stable IDs, deterministic CLI output. See [ADR-010](docs/ADR-010-stubborn-integration.md).
